...
ClientManager::ClientManager()
{
    ...
    moshou_stream = new EQ::Net::EQStreamManager(titanium_opts);
    moshou_ops = new RegularOpcodeManager;
    if (!moshou_ops->LoadOpcodes(server.config->GetVariable("Titanium", "opcodes").c_str()))
	{
    ...
    moshou_stream->OnNewConnection([this](std::shared_ptr<EQ::Net::EQStream> stream) {
        LogF(Logs::General, Logs::Login_Server, "New moshou client connection from {0}:{1}", stream->RemoteEndpoint(), stream->GetRemotePort());
        stream->SetOpcodeManager(&moshou_ops);
        Client *c = new Client(stream, this);
		clients.push_back(c);
	});
}
...
void ClientManager::ProcessClientEnterWorld(Client *oc)
{
    auto iter = clients.begin();
    while (iter != clients.end())
    {
        if((*iter) != oc)
        {
            (*iter)->processAClientEnter(oc);
            oc->loadOtherClient(*iter);
        }
        ++iter;
    }
}

void ClientManager::ProcessClientUpdate(Client *oc)
{
    auto iter = clients.begin();
    while (iter != clients.end())
    {
